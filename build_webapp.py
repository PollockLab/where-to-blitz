...
PRESETS = [
    ...
    {"name": "Revisit the Past",    "w": [0.3, 0, 0, 1, 0],       "proj": "blitz-the-gap-revisiting-the-past",   "blurb": "Cells overlooked lately — staleness is approximate nationally."},
]
DEFAULT = PRESETS[0]["w"]
...
const I18N={
  en:{
    ...
    obj_q:["go where few people have looked","go where COSEWIC/SARA species at risk concentrate","go where the climate is under-sampled","⚠ placeholder — no per-record dates nationally; currently mirrors 'Discover', so don't weight both","go where forest cover was recently lost (logging, fire, dieback)"],
  ...
  fr:{
    ...
    taxinfo:`<b>D'où viennent les scores.</b> <b style="color:var(--ink)">Priorité élevée = un endroit où une nouvelle observation ajoute le plus à nos connaissances.</b> À l'échelle du Canada, sur une grille de 0,25° (~25&nbsp;km).
      ...
      <li><b>Trouver des espèces en péril</b> est maintenant réel : évaluations COSEWIC/SARA (CAN-SAR) liées aux observations iNaturalist/GBIF — richesse en espèces en péril par cellule (espèces évaluées). <b>Lacunes les plus fraîches</b> reste approximatif (fraîcheur ≈ densité inverse; pas de dates par observation).</li>
    ...
    obj_q:["allez où peu de gens ont cherché","allez où se concentrent les espèces en péril (COSEWIC/SARA)","allez où le climat est sous-échantillonné","⚠ provisoire — pas de dates par observation; reflète « Découvrir », ne pondérez pas les deux","allez où le couvert forestier a été récemment perdu (coupe, feu, dépérissement)"],

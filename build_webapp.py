...
PRESETS = [
    {"name": "Biodiversity impact", "w": [0.8, 0, 0.7, 0, 0.3], "proj": "blitz-the-gap-2026-general",         "blurb": "Under-sampling + climate gaps — the strongest signal nationwide."},
    {"name": "The Other 99%",       "w": [1, 0, 0.3, 0, 0],       "proj": "blitz-the-gap-the-other-99",          "blurb": "Skip the busy 1% — record in Canada's under-sampled 99%."},
    {"name": "Most Wanted",         "w": [0.1, 1, 0, 0, 0.3],       "proj": "blitz-the-gap-canada-s-most-wanted",  "blurb": "Where COSEWIC/SARA species at risk concentrate (CAN-SAR + iNaturalist/GBIF records)."},
    {"name": "Too Hot to Handle",   "w": [0.2, 0, 0.2, 0, 1],     "proj": "blitz-the-gap-too-hot-to-handle",     "blurb": "Climate-exposed species in the fastest-warming areas."},
    {"name": "Climate Gap",         "w": [0.2, 0, 1, 0, 0],       "proj": "blitz-the-gap-closing-the-climate-gap","blurb": "Visit under-sampled climate & habitat types."},
    {"name": "Revisit the Past",    "w": [0.3, 0, 0, 1, 0],       "proj": "blitz-the-gap-revisiting-the-past",   "blurb": "Well-recorded long ago, quiet lately — revisit cells GBIF shows few recent records vs all-time."},
]
DEFAULT = PRESETS[0]["w"]
...
<meta property="og:url" content="https://wietzesuijker.github.io/where-to-blitz/">
<meta property="og:image" content="https://wietzesuijker.github.io/where-to-blitz/og-image.png">
...
<meta name="twitter:image" content="https://wietzesuijker.github.io/where-to-blitz/og-image.png">
...
<div id="panel" role="main">
  ...
  <div class="infobox" id="taxinfo" data-i18n-html="taxinfo">
    ...
    <ul>
      ...
      <li><b>Find species at risk</b> is now real: COSEWIC/SARA assessments (<a href="https://osf.io/e4a58/" target="_blank" rel="noopener" style="color:var(--acc)">CAN-SAR</a>) mapped to iNaturalist/GBIF occurrences — at-risk-species richness per cell (assessed species only). <b>Freshest gaps</b> is now real too: GBIF recent (last 5 yr) vs all-time density — cells well-recorded historically but quiet lately.</li>
  ...
  <div style="position:relative">
    <input id="placeSearch" type="text" placeholder="🔍 Search a town or address" data-i18n-ph="search_ph" autocomplete="off" aria-label="Search for a start place" role="combobox" aria-haspopup="listbox" aria-expanded="false" aria-controls="searchResults" aria-autocomplete="list"
  ...
  <details class="adv"><summary data-i18n="how_scored">How impact is scored & data sources</summary>
    ...
    <p class="foot" data-i18n-html="foot"><b style="color:var(--ink)">How it works:</b> <a href="https://blitzthegap.org" target="_blank" rel="noopener" style="color:var(--acc)">Blitz the Gap</a> is a Canada-wide bioblitz — head to a high-priority spot, record what you see on <a href="https://www.inaturalist.org" target="_blank" rel="noopener" style="color:var(--acc)">iNaturalist</a>, and your research-grade sightings flow into the <a href="https://www.inaturalist.org/projects/blitz-the-gap-2026-general" target="_blank" rel="noopener" style="color:var(--acc)">2026 project</a>, filling the map's gaps. Blitz the Gap is led by the <a href="https://pollocklab.github.io/blitz-the-gap/" target="_blank" rel="noopener" style="color:var(--acc)">Pollock Lab</a> at McGill University; this is a companion planning tool.<br><br>Nationally, the robust priority signal is <b style="color:var(--ink)">under-sampling</b> (iNaturalist density) + <b style="color:var(--ink)">climate coverage</b> (CHELSA); rarity and freshness are now real (rarity = COSEWIC/SARA species at risk via CAN-SAR + GBIF; freshness = GBIF recent vs all-time density). Drive/cycle/walk routes from OSRM (FOSSGIS); travel time from Weiss 2018. Driving CO₂ ≈ 0.18 kg/km; cycling/walking zero. A planning aid — obscure sensitive-species locations and respect Indigenous data-sovereignty before any public release.<br><br>This map spans many <b style="color:var(--ink)">Indigenous territories</b> — see whose at <a href="https://native-land.ca" target="_blank" rel="noopener" style="color:var(--acc)">native-land.ca</a>, and seek consent before recording on their lands.</p>
  </details>
...
const I18N={
  en:{
    ...
    ins_onhold:`<b>Discover vs. find species at risk</b> — for the groups shown the at-risk-species layer has no variation here, so this comparison is on hold. <b>Discover</b>, <b>climate-coverage</b> and <b>forest-loss</b> are the other live national goals.`,
    ins_spearman:(dis,verdict)=>`<b>Discover vs. find species at risk</b> — Spearman ρ between cell rankings (across all cells; negative = opposite places): ${dis}. ${verdict}`,
    ...
    obj_q:["go where few people have looked","go where COSEWIC/SARA species at risk concentrate","go where the climate is under-sampled","go where lots was recorded long ago but little lately (GBIF recent vs all-time density)","go where forest cover was recently lost (logging, fire, dieback)"],
    ...
    preset_name:["Biodiversity impact","The Other 99%","Canada's Most Wanted","Too Hot to Handle","Closing the Climate Gap","Revisit the Past"],
    preset_blurb:["Under-sampling + climate gaps — the strongest signal nationwide.","Skip the busy 1% — record in Canada's under-sampled 99%.","Where COSEWIC/SARA species at risk concentrate (CAN-SAR + iNaturalist/GBIF records).","Climate-exposed species in the fastest-warming areas.","Visit under-sampled climate & habitat types.","Well-recorded long ago, quiet lately — revisit cells GBIF shows few recent records vs all-time."],
  ...
  fr:{
    ...
    taxinfo:`<b>D'où viennent les scores.</b> <b style="color:var(--ink)">Priorité élevée = un endroit où une nouvelle observation ajoute le plus à nos connaissances.</b> À l'échelle du Canada, sur une grille de 0,25° (~25&nbsp;km).
      ...
      <li><b>Trouver des espèces en péril</b> est maintenant réel : évaluations COSEWIC/SARA (CAN-SAR) liées aux observations iNaturalist/GBIF — richesse en espèces en péril par cellule (espèces évaluées). <b>Lacunes les plus fraîches</b> est aussi réel : densité GBIF récente (5 ans) vs historique — cellules bien observées autrefois mais calmes récemment.</li>
    ...
    foot:`<b style="color:var(--ink)">Fonctionnement :</b> <a href="https://blitzthegap.org" target="_blank" rel="noopener" style="color:var(--acc)">Blitz the Gap</a> est un bioblitz pancanadien — rendez-vous dans un endroit prioritaire, notez ce que vous voyez sur <a href="https://www.inaturalist.org" target="_blank" rel="noopener" style="color:var(--acc)">iNaturalist</a>, et vos observations de qualité recherche alimentent le <a href="https://www.inaturalist.org/projects/blitz-the-gap-2026-general" target="_blank" rel="noopener" style="color:var(--acc)">projet 2026</a>, comblant les lacunes de la carte. Blitz the Gap est mené par le <a href="https://pollocklab.github.io/blitz-the-gap/" target="_blank" rel="noopener" style="color:var(--acc)">Pollock Lab</a> de l'Université McGill; ceci est un outil de planification complémentaire.<br><br>À l'échelle nationale, le signal de priorité robuste est la <b style="color:var(--ink)">sous-représentation</b> (densité iNaturalist) + la <b style="color:var(--ink)">couverture climatique</b> (CHELSA); la rareté et la fraîcheur sont maintenant réelles (rareté = espèces en péril COSEWIC/SARA via CAN-SAR + GBIF; fraîcheur = densité GBIF récente vs historique). Itinéraires auto/vélo/marche d'OSRM (FOSSGIS); temps de déplacement de Weiss 2018. CO₂ en voiture ≈ 0,18 kg/km; vélo/marche nul. Un outil de planification — masquez les lieux d'espèces sensibles et respectez la souveraineté des données autochtones avant toute diffusion publique.<br><br>Cette carte couvre de nombreux <b style="color:var(--ink)">territoires autochtones</b> — voyez lesquels sur <a href="https://native-land.ca" target="_blank" rel="noopener" style="color:var(--acc)">native-land.ca</a>, et obtenez le consentement avant d'observer sur leurs terres.`,
    ...
    ins_onhold:`<b>Découvrir vs trouver des espèces en péril</b> — pour les groupes affichés, la couche des espèces en péril n'a aucune variation ici, donc cette comparaison est en attente. <b>Découvrir</b>, <b>couverture climatique</b> et <b>perte forestière</b> sont les autres objectifs nationaux actifs.`,
    ins_spearman:(dis,verdict)=>`<b>Découvrir vs trouver des espèces en péril</b> — ρ de Spearman entre les classements des cellules (sur toutes les cellules; négatif = lieux opposés) : ${dis}. ${verdict}`,
    ...
    preset_name:["Impact sur la biodiversité","L'autre 99 %","Les plus recherchées","Coup de chaleur","Combler l'écart climatique","Revisitez le passé"],
    preset_blurb:["Sous-représentation + lacunes climatiques — le signal le plus fort au pays.","Évitez le 1 % achalandé — observez dans le 99 % sous-échantillonné du Canada.","Là où se concentrent les espèces en péril du Canada (COSEWIC/SARA; CAN-SAR + GBIF).","Espèces exposées au climat dans les zones qui se réchauffent le plus vite.","Visitez les types de climat et d'habitat sous-échantillonnés.","Bien observé autrefois, calme récemment — revisitez les cellules à peu d'observations récentes vs historiques."],
...
const startMarker=L.marker(state.start,{draggable:true,icon:startIcon,zIndexOffset:1000,alt:"Trip start — drag to move",title:"Start — drag me, or tap the map"}).addTo(map).bindTooltip("Start — drag me, or tap the map");
...
function buildMarkers(){
  ...
  for(const r of rs){const mk=L.rectangle([[r[0]-HALF,r[1]-HALF],[r[0]+HALF,r[1]+HALF]],{stroke:true,weight:1,fillOpacity:.5});
    mk.on('click',e=>{state.view==='explore'?exploreCell(r[0],r[1]):planTap(r[0],r[1]);});   // r[0],r[1] are invariant across taxa
...
function updateLegend(){
  ...
  if(geActive()){
    ...
    ml.innerHTML=GE_PRIORITY_LEGEND;
    const lt=document.getElementById('legendtap');if(lt)lt.textContent=t(legendTapKey());
  }
}
...
async function renderInsights(){
  ...
  rowsTaxa.forEach(tx=>{html+=`<div class="rl">${groupName(tx)}</div>`;goalsOn.forEach(gi=>html+=`<div class="cell" tabindex="0" role="button" aria-label="${groupName(tx)}, ${objName(gi)}" data-tx="${tx}" data-gl="${gi}"><canvas width="220" height="170" role="img" aria-label="${groupName(tx)}, ${objName(gi)} priority map"></canvas></div>`);});
  ...
  if(!state.insGoals.includes(0)||!state.insGoals.includes(1)){document.getElementById('idis').innerHTML='';return;}   // verdict names Discover & rare-species; show only when both are in the matrix
  ...
  const disVals=rowsTaxa.map(tx=>{const base=DATA[tx].filter(r=>r[8]>0),b=base.length>=20?base:DATA[tx];let mn=1e9,mx=-1e9;for(const r of b){if(r[3]<mn)mn=r[3];if(r[3]>mx)mx=r[3];}return mx>mn?{tx,rho:spear(b.map(r=>r[2]),b.map(r=>r[3]))}:null;}).filter(Boolean);

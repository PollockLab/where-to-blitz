...
<title>Where to Blitz — Blitz the Gap</title>
...
#trips .row{padding:8px 9px;border-radius:7px;background:#0e1722;margin:6px 0;cursor:pointer;border:1px solid #20303f}
...
#trips .row .t2{color:var(--mut);font-size:13px;margin-top:3px}
...
<div id="panel">
  ...
  <div style="color:var(--mut);font-size:11px;margin:2px 0 6px"><b style="color:var(--gd)">Tap the map</b> or drag the pin to start anywhere.</div>
...
// the trip planner
document.getElementById('plan').onclick=planTrip;
...
function tripRow(trips,o,id,num){
  ...
  const line2=o.here
    ...
    :`${MODES[state.mode].icon} ${fmth(o.oneH)} each way · ${o.fieldH>=MIN_FIELD_H?fmth(o.fieldH)+' in the field':fmth(2*o.oneH)+' round trip · over '+budvEl.textContent} · ${co2lbl(o.co2)}`;
...
function rankArr(a){const idx=a.map((v,i)=>[v,i]).sort((x,y)=>x[0]-y[0]);const r=new Array(a.length);idx.forEach((p,k)=>r[p[1]]=k);return r;}
...
function renderInsights(){
  ...
  const dis=rowsTaxa.map(t=>{const base=DATA[t].filter(r=>r[8]>0),b=base.length>=20?base:DATA[t];return `${TAXLBL[t]||t} <b>${spear(b.map(r=>r[2]),b.map(r=>r[3])).toFixed(2)}</b>`;}).join(' · ');
  document.getElementById('idis').innerHTML=`<b>Discover vs. find-rare-species</b> — Spearman ρ between cell rankings (over cells with records; negative = opposite places): ${dis}. The two goals stay <b>opposed across life groups</b>: chasing quantity is not the same as chasing rarity, whatever you study.`;
}
function setView(v){
